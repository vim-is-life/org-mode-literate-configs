...
We enable checking for changes, repo changes, and set the format of ~vcs_info~.
#+begin_src bash
...
Setup the default colorful prompt, and then add vcs info.
#+begin_src bash
PS1="%B%{$fg[green]%}[%{$fg[yellow]%}%n%{$fg[green]%}@%{$fg[blue]%}%M %{$fg[magenta]%}%~"
PS1="${PS1} %F{red}${vcs_info_msg_0_}%f"
